###############################################################################################
# This file is to implement Non-linear Support Vector Classifier on the different datasets    #
###############################################################################################
import pandas as pd
import numpy as np
...
patch_sklearn() # this must be imported put before importing sklearn
...
target_count_raw = raw.readmission_90.value_counts()
target_count_raw.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_raw[0])
print('Class 1:', target_count_raw[1])
print('Proportion:', round(target_count_raw[0] / target_count_raw[1], 2), ': 1') # (5.71 : 1)
raw.columns
...
###########################################
# Implementing Non-linear SVC on Raw
###########################################
patch_sklearn() 
from sklearn.svm import NuSVC
SVM_raw = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_raw.fit(X_raw_clean_train, y_raw_train)
y_pred_raw = SVM_raw.predict(X_raw_clean_test)
print("Accuracy of Support Vector Machine Raw:", metrics.accuracy_score(y_raw_test, y_pred_raw))
print("Precision:",metrics.precision_score(y_raw_test, y_pred_raw))
print("Recall:",metrics.recall_score(y_raw_test, y_pred_raw))
print(classification_report(y_raw_test, y_pred_raw))

# ROC curve
#--------------------
y_pred_proba_raw = SVM_raw.predict_proba(X_raw_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_raw_test, y_pred_proba_raw)
AUC = metrics.roc_auc_score(y_raw_test, y_pred_proba_raw)
...
#----------------------------------------
y_pred_raw_CI = np.array(y_pred_raw)
y_raw_true = np.array(y_raw_test)
y_pred_proba_raw_CI =np.array(y_pred_proba_raw)
n_bootstraps = 1000
rng_seed = 42  
...
for i in range(n_bootstraps):
    ...
    accuracy_raw = accuracy_score(y_raw_true[indices_raw], y_pred_raw_CI[indices_raw])
    precision_raw = precision_score(y_raw_true[indices_raw], y_pred_raw_CI[indices_raw])
    recall_raw = recall_score(y_raw_true[indices_raw], y_pred_raw_CI[indices_raw])
    f1_raw = f1_score(y_raw_true[indices_raw], y_pred_raw_CI[indices_raw])
    auc_raw = roc_auc_score(y_raw_true[indices_raw], y_pred_proba_raw_CI[indices_raw])
...
#-----------------------------------------
cm_raw= metrics.confusion_matrix(y_raw_test, y_pred_raw)
...
target_count_R1 = R1.readmission_90.value_counts()
target_count_R1.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_R1[0])
print('Class 1:', target_count_R1[1])
print('Proportion:', round(target_count_R1[0] / target_count_R1[1], 2), ': 1') # (6.3 : 1)
R1.columns
...
y_pred_R1 = SVM_R1.predict(X_R1_clean_test)
print("Accuracy of Support Vector Machine R1:", metrics.accuracy_score(y_R1_test, y_pred_R1))
print("Precision:",metrics.precision_score(y_R1_test, y_pred_R1))
print("Recall:",metrics.recall_score(y_R1_test, y_pred_R1))
print(classification_report(y_R1_test, y_pred_R1))

# ROC curve
#--------------------
y_pred_proba_R1 = SVM_R1.predict_proba(X_R1_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_R1_test, y_pred_proba_R1)
AUC = metrics.roc_auc_score(y_R1_test, y_pred_proba_R1)
...
## Calculate confidence interval for the metrics
#----------------------------------------
y_pred_R1_CI = np.array(y_pred_R1)
y_R1_true = np.array(y_R1_test)
y_pred_proba_R1_CI =np.array(y_pred_proba_R1)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_R1 = accuracy_score(y_R1_true[indices_R1], y_pred_R1_CI[indices_R1])
    precision_R1 = precision_score(y_R1_true[indices_R1], y_pred_R1_CI[indices_R1])
    recall_R1 = recall_score(y_R1_true[indices_R1], y_pred_R1_CI[indices_R1])
    f1_R1 = f1_score(y_R1_true[indices_R1], y_pred_R1_CI[indices_R1])
    auc_R1 = roc_auc_score(y_R1_true[indices_R1], y_pred_proba_R1_CI[indices_R1])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_R1= metrics.confusion_matrix(y_R1_test, y_pred_R1)
...
target_count_R08 = R08.readmission_90.value_counts()
target_count_R08.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_R08[0])
print('Class 1:', target_count_R08[1])
print('Proportion:', round(target_count_R08[0] / target_count_R08[1], 2), ': 1') # (5.99 : 1)
R08.columns
...
###########################################
# Implementing Non-linear SVC on R08
###########################################

SVM_R08 = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_R08.fit(X_R08_clean_train, y_R08_train)
y_pred_R08 = SVM_R08.predict(X_R08_clean_test)
print("Accuracy of Support Vector Machine R08:", metrics.accuracy_score(y_R08_test, y_pred_R08))
print("Precision:",metrics.precision_score(y_R08_test, y_pred_R08))
print("Recall:",metrics.recall_score(y_R08_test, y_pred_R08))
print(classification_report(y_R08_test, y_pred_R08))

# ROC curve
#--------------------
y_pred_proba_R08 = SVM_R08.predict_proba(X_R08_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_R08_test, y_pred_proba_R08)
AUC = metrics.roc_auc_score(y_R08_test, y_pred_proba_R08)
plt.plot(fpr,tpr,label="SVM_R08, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_R08_CI = np.array(y_pred_R08)
y_R08_true = np.array(y_R08_test)
y_pred_proba_R08_CI =np.array(y_pred_proba_R08)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_R08 = accuracy_score(y_R08_true[indices_R08], y_pred_R08_CI[indices_R08])
    precision_R08 = precision_score(y_R08_true[indices_R08], y_pred_R08_CI[indices_R08])
    recall_R08 = recall_score(y_R08_true[indices_R08], y_pred_R08_CI[indices_R08])
    f1_R08 = f1_score(y_R08_true[indices_R08], y_pred_R08_CI[indices_R08])
    auc_R08 = roc_auc_score(y_R08_true[indices_R08], y_pred_proba_R08_CI[indices_R08])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_R08= metrics.confusion_matrix(y_R08_test, y_pred_R08)
print('Confusion matrix:', cm_R08)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_R08', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
...
target_count_R06 = R06.readmission_90.value_counts()
target_count_R06.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_R06[0])
print('Class 1:', target_count_R06[1])
print('Proportion:', round(target_count_R06[0] / target_count_R06[1], 2), ': 1') # (5.82 : 1)
R06.columns
...
###########################################
# Implementing Non-linear SVC on R06
###########################################

SVM_R06 = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_R06.fit(X_R06_clean_train, y_R06_train)
y_pred_R06 = SVM_R06.predict(X_R06_clean_test)
print("Accuracy of Support Vector Machine R06:", metrics.accuracy_score(y_R06_test, y_pred_R06))
print("Precision:",metrics.precision_score(y_R06_test, y_pred_R06))
print("Recall:",metrics.recall_score(y_R06_test, y_pred_R06))
print(classification_report(y_R06_test, y_pred_R06))

# ROC curve
#--------------------
y_pred_proba_R06 = SVM_R06.predict_proba(X_R06_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_R06_test, y_pred_proba_R06)
AUC = metrics.roc_auc_score(y_R06_test, y_pred_proba_R06)
plt.plot(fpr,tpr,label="SVM_R06, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_R06_CI = np.array(y_pred_R06)
y_R06_true = np.array(y_R06_test)
y_pred_proba_R06_CI =np.array(y_pred_proba_R06)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_R06 = accuracy_score(y_R06_true[indices_R06], y_pred_R06_CI[indices_R06])
    precision_R06 = precision_score(y_R06_true[indices_R06], y_pred_R06_CI[indices_R06])
    recall_R06 = recall_score(y_R06_true[indices_R06], y_pred_R06_CI[indices_R06])
    f1_R06 = f1_score(y_R06_true[indices_R06], y_pred_R06_CI[indices_R06])
    auc_R06 = roc_auc_score(y_R06_true[indices_R06], y_pred_proba_R06_CI[indices_R06])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_R06= metrics.confusion_matrix(y_R06_test, y_pred_R06)
print('Confusion matrix:', cm_R06)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_R06', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
...
target_count_R05 = R05.readmission_90.value_counts()
target_count_R05.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_R05[0])
print('Class 1:', target_count_R05[1])
print('Proportion:', round(target_count_R05[0] / target_count_R05[1], 2), ': 1') # (5.81 : 1)
R05.columns
...
###########################################
# Implementing Non-linear SVC on R05
###########################################
SVM_R05 = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_R05.fit(X_R05_clean_train, y_R05_train)
y_pred_R05 = SVM_R05.predict(X_R05_clean_test)
print("Accuracy of Support Vector Machine R05:", metrics.accuracy_score(y_R05_test, y_pred_R05))
print("Precision:",metrics.precision_score(y_R05_test, y_pred_R05))
print("Recall:",metrics.recall_score(y_R05_test, y_pred_R05))
print(classification_report(y_R05_test, y_pred_R05))

# ROC curve
#--------------------
y_pred_proba_R05 = SVM_R05.predict_proba(X_R05_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_R05_test, y_pred_proba_R05)
AUC = metrics.roc_auc_score(y_R05_test, y_pred_proba_R05)
plt.plot(fpr,tpr,label="SVM_R05, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_R05_CI = np.array(y_pred_R05)
y_R05_true = np.array(y_R05_test)
y_pred_proba_R05_CI =np.array(y_pred_proba_R05)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_R05 = accuracy_score(y_R05_true[indices_R05], y_pred_R05_CI[indices_R05])
    precision_R05 = precision_score(y_R05_true[indices_R05], y_pred_R05_CI[indices_R05])
    recall_R05 = recall_score(y_R05_true[indices_R05], y_pred_R05_CI[indices_R05])
    f1_R05 = f1_score(y_R05_true[indices_R05], y_pred_R05_CI[indices_R05])
    auc_R05 = roc_auc_score(y_R05_true[indices_R05], y_pred_proba_R05_CI[indices_R05])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_R05= metrics.confusion_matrix(y_R05_test, y_pred_R05)
print('Confusion matrix:', cm_R05)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_R05', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
...
target_count_R01 = R01.readmission_90.value_counts()
target_count_R01.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_R01[0])
print('Class 1:', target_count_R01[1])
print('Proportion:', round(target_count_R01[0] / target_count_R01[1], 2), ': 1') # (5.62 : 1)
R01.columns
...
###########################################
# Implementing Non-linear SVC on R01
###########################################
SVM_R01 = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_R01.fit(X_R01_clean_train, y_R01_train)
y_pred_R01 = SVM_R01.predict(X_R01_clean_test)
print("Accuracy of Support Vector Machine R01:", metrics.accuracy_score(y_R01_test, y_pred_R01))
print("Precision:",metrics.precision_score(y_R01_test, y_pred_R01))
print("Recall:",metrics.recall_score(y_R01_test, y_pred_R01))
print(classification_report(y_R01_test, y_pred_R01))

# ROC curve
#--------------------
y_pred_proba_R01 = SVM_R01.predict_proba(X_R01_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_R01_test, y_pred_proba_R01)
AUC = metrics.roc_auc_score(y_R01_test, y_pred_proba_R01)
plt.plot(fpr,tpr,label="SVM_R01, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_R01_CI = np.array(y_pred_R01)
y_R01_true = np.array(y_R01_test)
y_pred_proba_R01_CI =np.array(y_pred_proba_R01)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_R01 = accuracy_score(y_R01_true[indices_R01], y_pred_R01_CI[indices_R01])
    precision_R01 = precision_score(y_R01_true[indices_R01], y_pred_R01_CI[indices_R01])
    recall_R01 = recall_score(y_R01_true[indices_R01], y_pred_R01_CI[indices_R01])
    f1_R01 = f1_score(y_R01_true[indices_R01], y_pred_R01_CI[indices_R01])
    auc_R01 = roc_auc_score(y_R01_true[indices_R01], y_pred_proba_R01_CI[indices_R01])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_R01= metrics.confusion_matrix(y_R01_test, y_pred_R01)
print('Confusion matrix:', cm_R01)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_R01', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
...
target_count_R001 = R001.readmission_90.value_counts()
target_count_R001.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_R001[0])
print('Class 1:', target_count_R001[1])
print('Proportion:', round(target_count_R001[0] / target_count_R001[1], 2), ': 1') # (5.67 : 1)
R001.columns
...
###########################################
# Implementing Non-linear SVC on R001
###########################################
SVM_R001 = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_R001.fit(X_R001_clean_train, y_R001_train)
y_pred_R001 = SVM_R001.predict(X_R001_clean_test)
print("Accuracy of Support Vector Machine R001:", metrics.accuracy_score(y_R001_test, y_pred_R001))
print("Precision:",metrics.precision_score(y_R001_test, y_pred_R001))
print("Recall:",metrics.recall_score(y_R001_test, y_pred_R001))
print(classification_report(y_R001_test, y_pred_R001))

# ROC curve
#--------------------
y_pred_proba_R001 = SVM_R001.predict_proba(X_R001_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_R001_test, y_pred_proba_R001)
AUC = metrics.roc_auc_score(y_R001_test, y_pred_proba_R001)
plt.plot(fpr,tpr,label="SVM_R001, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_R001_CI = np.array(y_pred_R001)
y_R001_true = np.array(y_R001_test)
y_pred_proba_R001_CI =np.array(y_pred_proba_R001)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_R001 = accuracy_score(y_R001_true[indices_R001], y_pred_R001_CI[indices_R001])
    precision_R001 = precision_score(y_R001_true[indices_R001], y_pred_R001_CI[indices_R001])
    recall_R001 = recall_score(y_R001_true[indices_R001], y_pred_R001_CI[indices_R001])
    f1_R001 = f1_score(y_R001_true[indices_R001], y_pred_R001_CI[indices_R001])
    auc_R001 = roc_auc_score(y_R001_true[indices_R001], y_pred_proba_R001_CI[indices_R001])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_R001= metrics.confusion_matrix(y_R001_test, y_pred_R001)
print('Confusion matrix:', cm_R001)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_R001', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
...
target_count_CCS = CCS.readmission_90.value_counts()
target_count_CCS.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_CCS[0])
print('Class 1:', target_count_CCS[1])
print('Proportion:', round(target_count_CCS[0] / target_count_CCS[1], 2), ': 1') # (5.77 : 1)
CCS.columns
...
###########################################
# Implementing Non-linear SVC on CCS
###########################################
SVM_CCS = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_CCS.fit(X_CCS_clean_train, y_CCS_train)
y_pred_CCS = SVM_CCS.predict(X_CCS_clean_test)
print("Accuracy of Support Vector Machine CCS:", metrics.accuracy_score(y_CCS_test, y_pred_CCS))
print("Precision:",metrics.precision_score(y_CCS_test, y_pred_CCS))
print("Recall:",metrics.recall_score(y_CCS_test, y_pred_CCS))
print(classification_report(y_CCS_test, y_pred_CCS))

# ROC curve
#--------------------
y_pred_proba_CCS = SVM_CCS.predict_proba(X_CCS_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_CCS_test, y_pred_proba_CCS)
AUC = metrics.roc_auc_score(y_CCS_test, y_pred_proba_CCS)
plt.plot(fpr,tpr,label="SVM_CCS, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_CCS_CI = np.array(y_pred_CCS)
y_CCS_true = np.array(y_CCS_test)
y_pred_proba_CCS_CI =np.array(y_pred_proba_CCS)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_CCS = accuracy_score(y_CCS_true[indices_CCS], y_pred_CCS_CI[indices_CCS])
    precision_CCS = precision_score(y_CCS_true[indices_CCS], y_pred_CCS_CI[indices_CCS])
    recall_CCS = recall_score(y_CCS_true[indices_CCS], y_pred_CCS_CI[indices_CCS])
    f1_CCS = f1_score(y_CCS_true[indices_CCS], y_pred_CCS_CI[indices_CCS])
    auc_CCS = roc_auc_score(y_CCS_true[indices_CCS], y_pred_proba_CCS_CI[indices_CCS])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_CCS= metrics.confusion_matrix(y_CCS_test, y_pred_CCS)
print('Confusion matrix:', cm_CCS)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_CCS', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
...
target_count_ICD_HIGHEST = ICD_HIGHEST.readmission_90.value_counts()
target_count_ICD_HIGHEST.plot(kind='bar', title='Count (readmission_90)')
print('Class 0:', target_count_ICD_HIGHEST[0])
print('Class 1:', target_count_ICD_HIGHEST[1])
print('Proportion:', round(target_count_ICD_HIGHEST[0] / target_count_ICD_HIGHEST[1], 2), ': 1') # (5.99 : 1)
ICD_HIGHEST.columns
...
##############################################
# Implementing Non-linear SVC on ICD-highest
##############################################
SVM_ICD_HIGHEST = NuSVC(gamma="auto",probability=True, kernel = 'rbf', random_state=42)
SVM_ICD_HIGHEST.fit(X_ICD_HIGHEST_clean_train, y_ICD_HIGHEST_train)
y_pred_ICD_HIGHEST = SVM_ICD_HIGHEST.predict(X_ICD_HIGHEST_clean_test)
print("Accuracy of Support Vector Machine ICD_HIGHEST:", metrics.accuracy_score(y_ICD_HIGHEST_test, y_pred_ICD_HIGHEST))
print("Precision:",metrics.precision_score(y_ICD_HIGHEST_test, y_pred_ICD_HIGHEST))
print("Recall:",metrics.recall_score(y_ICD_HIGHEST_test, y_pred_ICD_HIGHEST))
print(classification_report(y_ICD_HIGHEST_test, y_pred_ICD_HIGHEST))

# ROC curve
#--------------------
y_pred_proba_ICD_HIGHEST = SVM_ICD_HIGHEST.predict_proba(X_ICD_HIGHEST_clean_test)[::,1]
fpr, tpr, _ = metrics.roc_curve(y_ICD_HIGHEST_test, y_pred_proba_ICD_HIGHEST)
AUC = metrics.roc_auc_score(y_ICD_HIGHEST_test, y_pred_proba_ICD_HIGHEST)
plt.plot(fpr,tpr,label="SVM_ICD_HIGHEST, AUC="+str(AUC))
plt.legend(loc=4)
plt.show()

## Calculate confidence interval for the metrics
#--------------------------------------------------------------
y_pred_ICD_HIGHEST_CI = np.array(y_pred_ICD_HIGHEST)
y_ICD_HIGHEST_true = np.array(y_ICD_HIGHEST_test)
y_pred_proba_ICD_HIGHEST_CI =np.array(y_pred_proba_ICD_HIGHEST)
n_bootstraps = 1000
rng_seed = 42  
rng = np.random.RandomState(rng_seed)
...
for i in range(n_bootstraps):
    ...
    accuracy_ICD_HIGHEST = accuracy_score(y_ICD_HIGHEST_true[indices_ICD_HIGHEST], y_pred_ICD_HIGHEST_CI[indices_ICD_HIGHEST])
    precision_ICD_HIGHEST = precision_score(y_ICD_HIGHEST_true[indices_ICD_HIGHEST], y_pred_ICD_HIGHEST_CI[indices_ICD_HIGHEST])
    recall_ICD_HIGHEST = recall_score(y_ICD_HIGHEST_true[indices_ICD_HIGHEST], y_pred_ICD_HIGHEST_CI[indices_ICD_HIGHEST])
    f1_ICD_HIGHEST = f1_score(y_ICD_HIGHEST_true[indices_ICD_HIGHEST], y_pred_ICD_HIGHEST_CI[indices_ICD_HIGHEST])
    auc_ICD_HIGHEST = roc_auc_score(y_ICD_HIGHEST_true[indices_ICD_HIGHEST], y_pred_proba_ICD_HIGHEST_CI[indices_ICD_HIGHEST])
...
# plot the confusion materix (2*2 tabel)
#-----------------------------------------
cm_ICD_HIGHEST= metrics.confusion_matrix(y_ICD_HIGHEST_test, y_pred_ICD_HIGHEST)
print('Confusion matrix:', cm_ICD_HIGHEST)
...
ax.xaxis.set_label_position("top")
plt.tight_layout()
plt.title('Confusion matrix SVM_ICD_HIGHEST', y=1.1)
plt.ylabel('Actual label')
plt.xlabel('Predicted label')
